...
import { getRecommendedUsers,getUserFriends ,getOutgoingFriendsReqs, sendFriendRequest } from '../libs/api';
       ...
       <div className='container mx-auto space-y-10'>
          <div className='flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4'>
            ...
            <Link to="/notification" className='btn btn-outline btn-sm'>
              <UserIcon className="mr-2 size-4"/>Friend Request
            </Link>
          </div>
          ...
          {loadingUsers ?(
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
              {recommendedUsers.map((user)=>{
                const hasRequestSent=outgoingRequestIds.has(user._id)
                return (
                  <div key={user._id} className='card bg-base-200 hover:shadow-lg transition-all duration-300'>
                      <div className='card-body p-5 space-y-4 '>
                       ...
                       {/* //Action button  */}
                       <button className={`btn w-full mt-2 rounded-3xl ${hasRequestSent ? 'btn-disabled' : 'btn-primary'}`} 
